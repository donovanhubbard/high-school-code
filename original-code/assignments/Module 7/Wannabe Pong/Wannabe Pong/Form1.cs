	...
	public class Form1 : System.Windows.Forms.Form
	{
		...
		protected override void OnKeyDown(KeyEventArgs e)
		{
			...
			if(key == Keys.K)
				...
				awayScore++;
			else if(key == Keys.Up && p2Y > 88)
				p2Y -= 40;
			else if(key == Keys.Down && p2Y < 258)
				p2Y += 40;
			else if(key == Keys.W && p1Y > 88)
				p1Y -= 40;
			else if(key == Keys.S && p1Y < 258)
				p1Y += 40;
			if(pause == true && key == Keys.Enter)
			{
				pause = false;
